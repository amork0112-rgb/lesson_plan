...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (typeof priority !== 'number' || priority < 1) {
    return NextResponse.json({ error: 'priority must be >= 1' }, { status: 400 });
  }
  if (typeof sessions_per_week !== 'number' || sessions_per_week < 1) {
    return NextResponse.json({ error: 'sessions_per_week must be >= 1' }, { status: 400 });
  }
  ...
  const { data, error } = await supabase
    .from('class_book_allocations')
    .insert({ class_id: id, book_id, priority, sessions_per_week })
